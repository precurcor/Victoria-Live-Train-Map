(function(){
  ...
  function renderInspector(){
    ...
    if(!items.length){host.innerHTML=`<span class="subtitle">Project overview</span><h2 style="margin-top:8px">Shape your network.</h2><p class="muted">Select a station, track connection or component to edit its dimensions and position.</p><div class="stats" style="margin-top:18px"><div class="stat"><strong>${generated?.leds.length.toLocaleString()}</strong><span>LED footprints</span></div><div class="stat"><strong>${model.nodes.filter(n=>n.name).length}</strong><span>Station labels</span></div></div><h3>Between stations</h3><p class="muted">Shift-select two consecutive stations, then use <strong>Rebuild span</strong> to set the exact number of intermediate LEDs on every parallel track.</p><h3>Keep the original</h3><p class="muted">The Kea controller, USB and power schematics are retained. Your source PCB is never overwritten.</p><div class="note warning">Assign the exact OLED and encoder footprints in Hardware when you have chosen the parts. Exported boards are unrouted.</div><button data-action="checks" class="wide">View design checks (${issues.length})</button><div class="actions"><button data-action="help">Quick guide</button><button data-action="new">Project presets</button></div>`;return;}
  ...
  const actions={
    ...
    help:()=>modal('Quick guide',`<p><strong>1. Shape the board.</strong> Open the Board tab, enter width and height in millimetres. Resizing never scales the LED footprints.</p><p><strong>2. Move stations.</strong> Find a station in the left panel. Drag its outlined track column; drag its name separately to position the label. Drag across empty canvas to select a group, or use Shift-click.</p><p><strong>3. Add more LEDs.</strong> Select two consecutive stations with Shift-click, then Rebuild span. Enter the number between the stations <em>on each track</em>. Seven intermediate columns on three tracks adds 21 LEDs.</p><p><strong>4. Shape tracks.</strong> Click a connection and drag its diamond. The inspector edits density, output chain and explicit lane mappings. Connect mode joins two existing stations.</p><p><strong>5. Position hardware.</strong> Use Hardware to locate the OLED, encoder, existing buttons, USB sockets and MCU. Select controller group to move its associated parts together. Assign a real KiCad footprint and enter verified pin nets to include new parts in both PCB and schematic. Enable Capacitors in Board to reposition them.</p><p><strong>6. Save and export.</strong> Save project downloads a reopenable JSON file. Export KiCad downloads a complete project and mapping files. Finish electrical design, routing, ERC and DRC before ordering.</p><table><tr><td>Wheel</td><td>Zoom at pointer</td></tr><tr><td>Space + drag / middle drag</td><td>Pan</td></tr><tr><td>Shift-click</td><td>Multi-select</td></tr><tr><td>Arrow keys / Shift + arrows</td><td>Nudge one / ten grid steps</td></tr><tr><td>Ctrl+Z / Ctrl+Shift+Z</td><td>Undo / redo</td></tr><tr><td>Ctrl+S / Ctrl+O</td><td>Save / open project</td></tr><tr><td>F / Escape</td><td>Fit selection / clear selection</td></tr></table><div class="note">No installation or internet connection is needed. Autosave is convenient but browser-specific; project JSON is your portable backup. Sample lights in Preview are not live data.</div>`)
  };
